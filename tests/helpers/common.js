...
class Data {
  ...
  async hasEvent(ethers, tx, eventNamePlusReturn) {
    ...
    let eventSig = ethers.utils.keccak256( ethers.utils.toUtf8Bytes(eventNamePlusReturn) );
    return receipt.logs.filter(x => x.topics[0] === eventSig);
  }

  async getEventArgs(ethers, tx, eventNamePlusReturn) {
    const receipt = await ethers.provider.getTransactionReceipt(tx.hash);
    let eventSig = ethers.utils.keccak256( ethers.utils.toUtf8Bytes(eventNamePlusReturn) );
    let logItem = receipt.logs.filter(x => x.topics[0] === eventSig);

    let abi = [ "event " + eventNamePlusReturn ];
    let iface = new ethers.utils.Interface(abi);
    let log = iface.parseLog(logItem);
    // const {donor, value, tokenID} = log.args;

    console.log(log);
  }

  // async getEventArgs(tx) {
